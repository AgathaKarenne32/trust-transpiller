...
// Mapeamento OWASP Top 10 + CLASP
data VulnKind
  = sqlInjection()          // A03:2021 - Injection
  | xss()                   // A03:2021 - Cross-Site Scripting
  | shellInjection()        // A03:2021 - Command Injection
  | ldapInjection()         // A03:2021 - LDAP Injection
  | pathTraversal()         // A05:2021 - Security Misconfiguration
  | openRedirect()          // A03:2021 - Server-Side Request Forgery / Redirects
  | brokenAuthentication()  // A07:2021 - Identification & Auth Failures
  | insecureDesign()        // A04:2021 - Insecure Design (Bypass de Controle)
...
VulnReport buildReport(str sinkCat, str procName, str blockLabel, int instrIdx,
  ...
  str msg = "Inseguro [<origins>] alcanca `<sinkTarget>` (Vulnerabilidade mapeada pelo framework)";
...
tuple[VulnKind, Severity] classifySink(str cat) {
  switch (cat) {
    ...
    case "LDAP_QUERY":       return <ldapInjection(),        critical()>; // Mapeamento OWASP A03
    case "CREDENTIAL_STORE": return <brokenAuthentication(), high()>;     // Mapeamento OWASP A07
    case "PRIVILEGE_BYPASS": return <insecureDesign(),       high()>;     // Mapeamento OWASP A04
...
str formatReport(AuditResult ar) {
  str sep = "---------------------------------------------\n";
  str out = sep;
  out    += "TRUST-TRANSPILER AUDIT REPORT (OWASP & CLASP EDITION)\n";
...
private str kindStr(VulnKind k) {
  switch (k) {
    case sqlInjection():         return "A03:2021 - SQL Injection";
    case xss():                  return "A03:2021 - Cross-Site Scripting (XSS)";
    case shellInjection():       return "A03:2021 - Shell Injection";
    case ldapInjection():        return "A03:2021 - LDAP Injection";
    case pathTraversal():        return "A05:2021 - Path Traversal";
    case openRedirect():         return "A03:2021 - Open Redirect";
    case brokenAuthentication(): return "A07:2021 - Broken Authentication";
    case insecureDesign():       return "A04:2021 - Insecure Design";
